...
<head>
    ...
    <title>Overview rate of staff in each branch</title>
...
<TR>
<!-- Row 1 -->
<TD align=left colspan=4 style="background: #c4af6b;height: 30px;padding-left:10px"> <span style="font-size:22px;color: #fff;text-shadow: 1px 1px 2px rgba(73,73,73,0.61);"><strong>  Average of overview rate in each branch</strong></span></TD>
</TR>
<tr>
<td style="background: #fff;padding: 20px;">
	
	<?php
 $sql = "SELECT b.BranchID, b.Name, AVG(r.RateOverview) AS RateOverview
                         FROM branch b, review r
                         WHERE b.BranchID = r.BranchID
                         GROUP BY b.BranchID
                         ORDER BY RateOverview DESC;";
       $result = mysqli_query($conn, $sql);
        $resultCheck = mysqli_num_rows($result);
        $chart_data ='';
            while($row = mysqli_fetch_assoc($result)){
                $chart_data .="{ Name:'".$row["Name"]."', RateOverview:".$row["RateOverview"]."}, ";
            }
            $chart_data = substr ($chart_data,0,-2);
        ?>
	
	<h3>Graph shows average of overview rate in each branch</h3>
