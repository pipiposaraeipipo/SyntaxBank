...
<head>
    ...
    <title>Total amount of deposit in each branch</title>
...
<TR>
<!-- Row 1 -->
<TD align=left colspan=4 style="background: #c4af6b;height: 30px;padding-left:10px"> <span style="font-size:22px;color: #fff;text-shadow: 1px 1px 2px rgba(73,73,73,0.61);"><strong> Total amount of deposit in each branch</strong></span></TD>
</TR>
<tr>
<td style="background: #fff;padding: 20px;">
	
	<?php
                $sql = "SELECT b.BranchID, b.Name, SUM(d.Amount) AS Totaldeposit
                        FROM depositwithdraw d, branch b, staff s
                        WHERE d.StaffID = s.StaffID AND s.BranchID = b.BranchID AND d.Type = 'Deposit'
                        GROUP BY b.BranchID
      ...
      $result = mysqli_query($conn, $sql);
        ...
        $chart_data ='';
            while($row = mysqli_fetch_assoc($result)){
                $chart_data .="{ Name:'".$row["Name"]."', Totaldeposit:".$row["Totaldeposit"]."}, ";
            }
            $chart_data = substr ($chart_data,0,-2);
        ?>
	
	<h3>Graph shows total amount of deposit in each branch</h3>
