...
<head>
    ...
    <title>Total amount of withdraw in each branch</title>
...
<TR>
<!-- Row 1 -->
<TD align=left colspan=4 style="background: #c4af6b;height: 30px;padding-left:10px"> <span style="font-size:22px;color: #fff;text-shadow: 1px 1px 2px rgba(73,73,73,0.61);"><strong> Total amount of withdraw in each branch</strong></span></TD>
</TR>
<tr>
<td style="background: #fff;padding: 20px;">
	
	<?php
                $sql = "SELECT b.BranchID, b.Name, SUM(d.Amount) AS Totalwithdraw
                        FROM depositwithdraw d, branch b, staff s
                        WHERE d.StaffID = s.StaffID AND s.BranchID = b.BranchID AND d.Type = 'Withdraw'
                        GROUP BY b.BranchID
        ...
        $chart_data ='';
            while($row = mysqli_fetch_assoc($result)){
                $chart_data .="{ Name:'".$row["Name"]."', Totalwithdraw:".$row["Totalwithdraw"]."}, ";
            }
            $chart_data = substr ($chart_data,0,-2);
        ?>
	
	<h3>Graph shows total amount of withdraw in each branch</h3>
 ...
 <center style="color:#fff"><div css="pet" style="letter-spacing: 0.4px; width:680px; height: 200px; overflow: auto; border: 1px solid;">
	 ...
	 <table>
            <tr> 
                ...
                <th>BranchID</th>
                <th>BranchName</th>
                <th>Total amount of withdraw</th>
            </tr>
 <?php
                $result = mysqli_query($conn, $sql);
                $resultCheck = mysqli_num_rows($result);
