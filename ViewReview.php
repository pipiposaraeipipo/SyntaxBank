...
<table border="0" cellpadding="0" style="overflow-p:auto;max-width:1200px;position:relative;margin-right:auto;margin-left:auto;margin-top:20px;background-color:#fff!important;-webkit-box-shadow: 2px 6px 5px 0px rgba(128,128,128,0.65);
-moz-box-shadow: 2px 6px 5px 0px rgba(128,128,128,0.65);
box-shadow: 2px 6px 5px 0px rgba(128,128,128,0.65);">

<TR>
<!-- Row 1 -->
<TD align=left colspan=4 style="background: #c4af6b;height: 30px;padding-left:10px"> <span style="font-size:22px;color: #fff;text-shadow: 1px 1px 2px rgba(73,73,73,0.61);"><strong> All Reviews</strong></span></TD>
</TR>

<TR>
<!-- Row 2 -->
<td style="background: #fff;padding: 20px 20px 20px 20px;">
 <center style="color:#fff"><div css="pet" style="letter-spacing: 0.4px; width:1180px; height: 480px; overflow: auto; border: 1px solid;">
            ...
            <?php
                ...
                if ($resultCheck > 0) {
                    while($row = mysqli_fetch_assoc($result)) {
                        $field1name = $row["ReviewID"];
                        $field2name = $row["CustomerID"];
                        $field3name = $row["BranchID"];
                        $field4name = $row["RateOverview"];
                        $field5name = $row["RateService"];
                        $field6name = $row["RateDuration"];
                        $field7name = $row["RateAtmosphere"];
                        $field8name = $row["RateTools"];
                        $field9name = $row["Comment"];
